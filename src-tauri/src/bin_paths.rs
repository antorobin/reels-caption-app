...
//
// Resolved once, at startup (`init`, called from `run()`'s setup hook,
// which is the one place in this crate that both runs before any command
// and has an `AppHandle` on hand), then cached — most ffmpeg call sites
// are plain helper functions several layers away from any command handler
// and would otherwise need an `AppHandle` threaded through just for this.
...
/// Falls back to the bare PATH-resolved command name if `init` hasn't run
/// yet (e.g. unit tests) — same behavior this crate had before bundling.
pub fn ffmpeg_path() -> &'static Path {
    FFMPEG_PATH.get_or_init(|| PathBuf::from("ffmpeg")).as_path()
}

pub fn ffprobe_path() -> &'static Path {
    FFPROBE_PATH.get_or_init(|| PathBuf::from("ffprobe")).as_path()
}

pub fn fonts_dir() -> &'static Path {
    FONTS_DIR.get_or_init(|| PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("resources").join("fonts")).as_path()
}
